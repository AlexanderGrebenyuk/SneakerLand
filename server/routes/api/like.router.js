...
const {Sneaker} = require('../../db/models')
...
router.get('/',  async (req, res) => {
    try {
      const likes = await Like.findAll(  { include: [
        { model: Sneaker }
  ...
  router.post('/', verifyAccessToken, async (req, res) => {
    try {
      ...
      like = await Like.findOne({ where: { userId: user.id, sneakerId: sneakerId } });
    ...
    router.delete('/:id', verifyAccessToken, async (req,res)=>{
      ...
      const { user } = res.locals;
      const { id } = req.params;
      const result = await Like.destroy({ where: { userId: user.id, sneakerId: id } });
